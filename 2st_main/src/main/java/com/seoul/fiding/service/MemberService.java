...
public interface MemberService {
	...
	public BoardVO selectOneBoard(int BD_NO,String id); //게시물 하나 보여주기
    ...
    public void deleteALike(LikeVO vo); //좋아요 객체 삭제
   
    public void insertFavor(FavorVO vo); //즐겨찾기 객체 저장
    public void deleteAFavor(FavorVO vo); //즐겨찾기 객체 삭제
	
	public void insertTour(TourVO vo); //관광지 정보 등록
	public void searchUp(int TL_NO); // 임시
	
    public int maxTour(); // 최근 추가한 관광지 정보 가져오기
    public TourVO selectOneTour(int TL_NO); // 관광지 하나 보여주기
    public List<BoardVO> selectToplike(SearchVO vo); //좋아요순으로 게시글보여주기
    
    public int selectBoardPaging9l(SearchVO vo); // 게시믈 총합 개수
    public int selectBoardByIdPaging9l(SearchVO vo); // 내 게시믈 총합 개수
    public int selectBoardByFavorPaging9l(SearchVO vo); // 내 즐겨찾기 총합 개수
    public int selectBoardBySearchPaging9l(SearchVO vo);
    public int selectToplikePaging9l(SearchVO vo); // 좋아요 게시 총합 개수
