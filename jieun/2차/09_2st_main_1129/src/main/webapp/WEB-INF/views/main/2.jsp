...
<script type="text/javascript">
			  ...
			  console.log("요청 성공", data);
		        ...
		        if (data.response.body.items && data.response.body.items.item) {
		            for (var i = 0; i < data.response.body.items.item.length; i++) {
		                ...
		                var articleHTML = '<article class="destination" id="a'+i+'">';
		                ...
		                articleHTML += '<input type="button" onclick="tourinsert(this)" value="리뷰 게시물 작성" />';
		                articleHTML += '<input type="hidden" class="category" value="' + item.cat1 + '" />';
		                articleHTML += '</article>';
	...
	function tourinsert(element) {
		console.log("사랑")
		var cat = $(element).parent().find(".category").val();
		console.log(cat);

		console.log(cat);
/* 		var data={
				TL_NO: $(element).parent().find("h2").text(),
				cat: cat,
				address: $(element).parent().find("p").text(),
				search_cnt: 0,
